...
from model_pytorch import dnn_4l, dnn_3l, dnn_2l, dnn_5l
...
def train(args):
    ...
    t_overall = time.time()
    print('\n DNN for UK Biobank with argument: ' + str(args))
    ...
    # load data
    # ['tes_phe', 'tra_phe', 'x_test', 'x_train_raw', 'y_test_different_set_phe', 'y_train_raw']
    ...
    trainloader = DataLoader(dset_train, batch_size=args.batch_size, shuffle=True, num_workers=1)
    dset_valid = ukbb_multi_task_dataset(x_valid, y_valid)
    validLoader = DataLoader(dset_valid, batch_size=args.batch_size, shuffle=True, num_workers=1)
    dset_test = ukbb_multi_task_dataset(x_test, y_test_different_set_phe)
    testLoader = DataLoader(dset_test, batch_size=args.batch_size, shuffle=False, num_workers=1)
    ...
    for run in range(runs):
        ...
        for epoch in range(epochs):
            ...
            file_trained = os.path.join(
                dir_model_temp, 'ukbb_dnn_run_' + str(run) + '_epoch_' +
                str(epoch) + '.pkl_torch')
